...
headers_csv = ['Company_name', 'City', 'Country', 'Business', 'Products', 'Tags', 'Fax', 'Telephone', 'Email',
               'Web_Site', 'WhatsApp', 'Mobile',
               'Linkedin']

with open('data_cards_Exporter.csv', 'w', encoding='utf-8') as file:
...
for i in range(1, 160):
    countri_id = i
    with open(f'exporter_countrys/cards_Exporter{i}.html', 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    for profile_link in data_campo_values_producer:
        ...
        if response.status_code == 200:
            ...
            with open('data_cards_Exporter.csv', 'a', encoding='utf-8') as file:
                writer = csv.writer(file)
                writer.writerow(
                    [Company_name, City, Country, Business, Products, Tags, Fax, Telephone, Email, Web_Site, WhatsApp,
                     Mobile,
                     Linkedin])
